...
export default class ReleasePreparation extends Session {
  ...
  getSecurityCherryPickTargets(vulnData, line) {
    ...
    for (const dep of getDependencyUpdates(vulnData.dependencies)) {
      const url = getPullRequestURLForLine(
  ...
  async cherryPickSecurityPRs() {
    ...
    for (const target of targets) {
      ...
      if (!amendAll) {
        const answer = await cli.promptSelect(
          `Would you like to amend ${target.label} (${target.url}) to the proposal?`,
          [
            { name: 'Yes', value: 'yes' },
            { name: 'No, skip this PR', value: 'no' },
            {
              name: 'Yes to all remaining PRs (no further prompts)',
              value: 'all'
            }
          ]);
  ...
  async getBranchDiff(opts) {
    ...
    if (opts.onlyNotableChanges) {
      ...
      if (isLTS) {
        const res = await sendRequest('https://nodejs.org/dist/index.json');
  ...
  async prepareLocalBranch() {
    ...
    // On security releases `branch` resolves to `releaseBranch`, which
    // tryResetBranch() needs. Derive it from the branch that is checked out:
    // the new version is only calculated below, and it may bump the major.
    this.releaseBranch = `v${match[1]}.x`;
    await this.tryResetBranch();
